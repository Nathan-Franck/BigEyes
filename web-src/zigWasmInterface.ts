...
function messageFromWasm(sourcePtr: number, sourceLen: number) {
  ...
  if (onMessage == null) console.log(str);
  else onMessage(str);
}
...
function errorFromWasm(sourcePtr: number, sourceLen: number) {
  ...
  console.log(str);
  if (onError == null) console.error(str);
  else onError(str);
}
...
const instance = (await init({
  ...
  exports: {
    ...
    callWithJson: (
      ...
      args_ptr: number,
      args_len: number,
    ) => void; // TODO: Generate from zig file
    allocUint8: (length: number) => number;
    memory: WebAssembly.Memory;
  };
};

export function callWasm<T extends keyof WasmInterface>(
  name: T,
  ...args: Parameters<WasmInterface[T]>
): { error: string } | ReturnType<WasmInterface[T]> {
...
export function sliceToString(slice: {
  ...
  const result = new TextDecoder().decode(
    new Uint8Array(instance.exports.memory.buffer, slice.ptr, slice.len),
  );
  console.log("Slice to string", slice, result);
  return result;
}

function sliceToArrayFunc<
  N extends string,
  T extends {
    new (buffer: ArrayBuffer, byteOffset: number, length: number): any;
  },
>(_: N, constructor: T) {
