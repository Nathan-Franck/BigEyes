...
export type AllowedKeys<Base, Condition> = FilterFlags<
  Base,
  Condition
>[keyof Base];

export type GLSLUnit = "float" | "vec2" | "vec3" | "vec4";
export type GLSLUniformUnit =
  | "float"
  | "vec2"
  | "vec3"
  | "vec4"
...
export type ShaderGlobals = {
  readonly [key: string]: Varying | Attribute | Uniform | Element | Output;
};
...
export namespace ShaderBuilder {
  ...
  export async function loadTexture(gl: WebGL2RenderingContext, url: string) {
    return await new Promise<Texture>((resolve) => {
      ...
      image.onload = () => {
        ...
        gl.texImage2D(
          gl.TEXTURE_2D,
          textureSettings.level,
          textureSettings.internalFormat,
          textureSettings.srcFormat,
          textureSettings.srcType,
          image,
        );
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.REPEAT);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.REPEAT);
  ...
  export function createElementBuffer(
    ...
    return {
      ...
      buffer,
      length: data.length,
      glType: data.BYTES_PER_ELEMENT == 2 ? "UNSIGNED_SHORT" : "UNSIGNED_INT",
    };
  }
  ...
  export function createFrameBuffer(
    ...
    if (frameBuffer == null) {
      throw new Error("FrameBuffer is null, this is not expected!");
    }
    ...
    if (depth) {
      ...
      if (depthBuffer == null) {
        throw new Error("DepthBuffer is null, this is not expected!");
      }
  ...
  export function renderMaterial<T extends ShaderGlobals>(
...

    {
      ...
      Object.entries(material.globals)
        .filter(
          (entry): entry is [string, Uniform] => entry[1].type == "uniform",
        )
        .reduce((textureIndex, entry) => {
          ...
          switch (global.unit) {
            case "sampler2D":
              const textures =
                global.count > 1 ? (data as Texture[]) : [data as Texture];
            ...
            case "float":
              gl.uniform1fv(
                uniformLocation,
                global.count > 1 ? (data as number[]) : [data as number],
              );
              break;
            case "vec2":
              gl.uniform2fv(
...

    {
      ...
      Object.entries(material.globals)
        .filter(
          (entry): entry is [(typeof entry)[0], Attribute] =>
    ...
    // 🖌 Draw the arrays/elements using size determined from aggregating buffers
    {
          ...
          (bufferCounts, entry) => {
            ...
            return global.instanced
              ? {
                  ...bufferCounts,
                  instance: Math.max(
                    bufferCounts.instance,
                    data.length / unitSize,
                  ),
                }
              : {
                  ...bufferCounts,
                  element: Math.max(
                    bufferCounts.element,
                    data.length / unitSize,
                  ),
                };
          },
      ...
      const elementEntry = Object.entries(material.globals).find(
        (entry): entry is [(typeof entry)[0], Element] =>
  ...
  export function cleanupResources<
    T extends Record<string, Texture | SizedBuffer | ElementBuffer>,
  >(gl: WebGL2RenderingContext, binds: T) {
    Object.values(binds).forEach((bind) => {
      if (
        "type" in bind &&
        (bind.type == "attribute" || bind.type == "element")
      ) {
